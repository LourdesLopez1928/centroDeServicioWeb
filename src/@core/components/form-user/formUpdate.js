...
import TableStickyUser from './TableUser'
...
const FormUpdateCustomer = ({idUser}) => {
  ...
  useEffect(async() => {
    ...
    const {user} = await userService.getById(sessionStorage.getItem('token'),idUser)
    setEmail(user?.email)
    setName(user?.name)
  ...
  const savedForm = async () => {
    ...
    const { status } = await userService.update(idUser,object,sessionToken)
    if (status === 'success') {
      setTimeout(() => {
        setLoading(false)
        setEmail('')
